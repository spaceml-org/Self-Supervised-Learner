...
#     )
    ...
    if patience > 0:
        ...
        trainer = Trainer(gpus=gpus, max_epochs = epochs, progress_bar_refresh_rate=5, callbacks=[cb], distributed_backend='dp' if args.gpus > 1 else None, logger = wandb_logger)
    else:
        trainer = Trainer(gpus=gpus, max_epochs = epochs, progress_bar_refresh_rate=5, callbacks = [None], distributed_backend='dp' if args.gpus > 1 else None, logger = wandb_logger)
