...
def load_model(args):
    ...
    # encoder specified
        ...
        args.encoder = encoders.resnet18(pretrained=False, first_conv=True, maxpool1=True,
        ...
        args.encoder = encoders.resnet50(pretrained=False, first_conv=True, maxpool1=True,
                                         return_all_feature_maps=False)
...
def cli_main():
    ...
    parser.add_argument("--withhold_split", default=0, type=float,
    ...
    if not (os.path.isdir(f"{args.DATA_PATH}/train") and os.path.isdir(
            f"{args.DATA_PATH}/val")) and args.val_split != 0 and args.VAL_PATH is None:
        ...
        splitfolders.ratio(args.DATA_PATH, output=f'./split_data_{log_name[:-5]}',
                           ratio=(1 - args.val_split - args.withhold_split, args.val_split, args.withhold_split),
                           seed=args.seed)
    ...
    trainer = pl.Trainer(gpus=args.gpus, max_epochs=args.epochs, progress_bar_refresh_rate=20, callbacks=cbs,
                         ...
                         sync_batchnorm=True if args.gpus > 1 else False, logger=wandb_logger, enable_pl_optimizer=True)
    trainer.fit(model)
