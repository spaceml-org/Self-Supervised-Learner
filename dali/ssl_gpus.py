...
class sslSIMCLR(SimCLR):
  ...
  def __init__(self, encoder, epochs, DATA_PATH, withhold, batch_size, val_split, hidden_dims, train_transform, val_transform, num_workers, gpus, **kwargs):
      #data stuff
  ...
  def setup(self, stage = None):
      ...
      #model stuff, the gpu here is only for batch size computation    
      ...
      train_pipeline = self.train_transform(DATA_PATH = f"{self.DATA_PATH}/train", input_height = 256, batch_size = self.batch_size, num_threads = 4, device_id = self.global_rank)
      print(f"{self.DATA_PATH}/train")
      val_pipeline = self.val_transform(DATA_PATH = f"{self.DATA_PATH}/val", input_height = 256, batch_size = self.batch_size, num_threads = 4, device_id = self.global_rank)
...
def cli_main():
    ...
    if patience > 0:
        ...
        trainer = Trainer(gpus=gpus, max_epochs = epochs, callbacks=[cb, online_evaluator], progress_bar_refresh_rate=5, distributed_backend='dp' if args.gpus > 1 else None)
    else:
        trainer = Trainer(gpus=gpus, max_epochs = epochs, callbacks = [online_evaluator], progress_bar_refresh_rate=5, distributed_backend='dp' if args.gpus > 1 else None)
