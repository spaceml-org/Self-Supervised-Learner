...
def cli_main():
    ...
    if patience > 0:
        ...
        trainer = Trainer(gpus=gpus, max_epochs = epochs, callbacks=[cb, online_evaluator], progress_bar_refresh_rate=5, logger = wandb_logger)
    else:
        trainer = Trainer(gpus=gpus, max_epochs = epochs, callbacks = [online_evaluator], progress_bar_refresh_rate=5, logger = wandb_logger)
